...
const AzureComponentSchema = z.object({
  ...
  serviceType: z.string().describe('The Azure service type (e.g., "Azure App Service", "Azure SQL Database").'),
  region: z.string().describe('The Azure region where the component is deployed (e.g., "East US 2").'),
  specifications: z.record(z.any()).describe('Specifications and projected usage for the component.').optional()
});
...
const AzureCostEstimationInputSchema = z.object({
  ...
  components: z.array(AzureComponentSchema).describe('An array of architectural components.')
});
...
const AzureCostEstimationOutputSchema = z.object({
  ...
  costBreakdown: z.array(z.object({
    ...
    details: z.string().describe('A detailed explanation of how the cost was estimated.')
  })).describe('A detailed breakdown of the estimated monthly cost for each architectural component.'),
  assumptions: z.string().describe('Any general assumptions made during the cost estimation process.')
});
